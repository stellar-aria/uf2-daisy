...
UINT USBD_DFU_CustomRequest(VOID *dfu_instance, UX_SLAVE_TRANSFER *transfer) {
  ...
  if ((dfu_state == UX_SYSTEM_DFU_STATE_DFU_IDLE) || (dfu_state == UX_SYSTEM_DFU_STATE_DFU_DNLOAD_IDLE)) {
    ...
    if (status == TX_SUCCESS) {
      ...
      /* Regular Download Command */
        ...
        for (Block_index = 0; Block_index < ux_dfu_download.wlength; Block_index += 32) {
          ...
          if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_FLASHWORD, Media_address, Data_address) == HAL_OK) {
